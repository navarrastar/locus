...
triangle :: proc(
    ...
    append(&every_vertex, 
        adjusted_v1.x, adjusted_v1.y, adjusted_v1.z, color.x, color.y, color.z, color.w, normal.x, normal.y, normal.z,
        adjusted_v2.x, adjusted_v2.y, adjusted_v2.z, color.x, color.y, color.z, color.w, normal.x, normal.y, normal.z,
        adjusted_v3.x, adjusted_v3.y, adjusted_v3.z, color.x, color.y, color.z, color.w, normal.x, normal.y, normal.z,
    )
    return Geometry {
...
capsule :: proc(
    ...
    // The bottom of the capsule should be at y=0 
    // Need to shift all y-coordinates up by (half_height + radius)
    y_offset: f32 = half_height + radius
    ...
    for ring := 0; ring < rings; ring += 1 {
        ...
        for segment := 0; segment < segments; segment += 1 {
            ...
            y1 := radius * m.sin(ring_angle1) + half_height + y_offset
            ...
            y2 := radius * m.sin(ring_angle1) + half_height + y_offset
            ...
            y3 := radius * m.sin(ring_angle2) + half_height + y_offset
            ...
            y4 := radius * m.sin(ring_angle2) + half_height + y_offset
    ...
    for segment := 0; segment < segments; segment += 1 {
        ...
        // First triangle
        append(&every_vertex,
            x1, half_height + y_offset, z1, color.x, color.y, color.z, color.w, nx1, 0, nz1,
            x2, half_height + y_offset, z2, color.x, color.y, color.z, color.w, nx2, 0, nz2,
            x2, -half_height + y_offset, z2, color.x, color.y, color.z, color.w, nx2, 0, nz2,
        )
...
sphere :: proc(
    ...
    // The bottom of the sphere should be at y=0
    // Need to shift all y-coordinates up by radius
    y_offset: f32 = radius
    
    // Generate vertices for the sphere
    for ring := 0; ring <= rings; ring += 1 {
        ring_angle := f32(ring) * m.PI / f32(rings)
        ...
        for segment := 0; segment < segments; segment += 1 {
            ...
            sin_ring := m.sin(ring_angle)
            cos_ring := m.cos(ring_angle)
            
            // Current ring
            x1 := radius * sin_ring * m.cos(segment_angle1)
            y1 := radius * cos_ring + y_offset
            z1 := radius * sin_ring * m.sin(segment_angle1)
            
            nx1 := sin_ring * m.cos(segment_angle1)
            ny1 := cos_ring
            nz1 := sin_ring * m.sin(segment_angle1)
            
            x2 := radius * sin_ring * m.cos(segment_angle2)
            y2 := radius * cos_ring + y_offset
            z2 := radius * sin_ring * m.sin(segment_angle2)
            
            nx2 := sin_ring * m.cos(segment_angle2)
            ny2 := cos_ring
            nz2 := sin_ring * m.sin(segment_angle2)
            
            // Skip vertex creation if we're at poles (prevents degenerate triangles)
            if ring < rings {
                // Next ring
                next_ring_angle := f32(ring + 1) * m.PI / f32(rings)
                sin_next_ring := m.sin(next_ring_angle)
                cos_next_ring := m.cos(next_ring_angle)
                
                x3 := radius * sin_next_ring * m.cos(segment_angle2)
                y3 := radius * cos_next_ring + y_offset
                z3 := radius * sin_next_ring * m.sin(segment_angle2)
                
                nx3 := sin_next_ring * m.cos(segment_angle2)
                ny3 := cos_next_ring
                nz3 := sin_next_ring * m.sin(segment_angle2)
                
                x4 := radius * sin_next_ring * m.cos(segment_angle1)
                y4 := radius * cos_next_ring + y_offset
                z4 := radius * sin_next_ring * m.sin(segment_angle1)
                
                nx4 := sin_next_ring * m.cos(segment_angle1)
                ny4 := cos_next_ring
                nz4 := sin_next_ring * m.sin(segment_angle1)
                
                // First triangle (except at north pole)
                if ring > 0 {
                    append(&every_vertex,
                        x1, y1, z1, color.x, color.y, color.z, color.w, nx1, ny1, nz1,
                        x2, y2, z2, color.x, color.y, color.z, color.w, nx2, ny2, nz2,
                ...
                // Second triangle (except at south pole)
                if ring < rings - 1 {
                    append(&every_vertex,
                        x1, y1, z1, color.x, color.y, color.z, color.w, nx1, ny1, nz1,
                        x3, y3, z3, color.x, color.y, color.z, color.w, nx3, ny3, nz3,
