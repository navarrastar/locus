...
renderer_init :: proc() {
	...
	assert(render_state.gpu != nil, "Failed to create GPU device")
...
renderer_setup_vertex_buffer :: proc(geom: ^Geometry) {
	...
	vertex_buffer := sdl.CreateGPUBuffer(gpu, {usage = {.VERTEX}, size = vertices_size})

	transfer_buffer := sdl.CreateGPUTransferBuffer(gpu, {usage = .UPLOAD, size = vertices_size})
...
renderer_draw_geometry :: proc(render_pass: ^sdl.GPURenderPass, geom: ^Geometry) {
	...
	sdl.BindGPUGraphicsPipeline(render_pass, geom.material.pipeline)
